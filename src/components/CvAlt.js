    ...
    return (
        <div className="min-h-screen bg-base-100 py-5 px-7 lg:max-w-5xl lg:mx-auto">
            <div className="flex flex-col gap-2 mb-4 items-center">
                <h1 className="text-5xl capitalize text-center"><span className="text-gray-700 dark:text-gray-500">{data.firstName}</span> <span className="font-bold">{data.lastName}</span></h1>
                <h3 className="text-lg uppercase text-gray-600 dark:text-gray-400">{data.title}</h3>
            </div>
            <div className="flex flex-row gap-3 flex-wrap shadow-lg rounded-lg p-4">
                <p className="flex flex-row gap-2 items-center text-sm md:text-md capitalize">
                ...
                <p className="flex flex-row gap-2 items-center text-sm md:text-md">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
                ...
                <p className="flex gap-2 items-center text-sm md:text-md">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 mt-8 gap-x-10 gap-y-8 print:grid-cols-2">
                <div className="flex flex-col gap-5">
                    <section className={`flex flex-col gap-2 shadow-lg rounded-lg`}>
                        <h2 className="text-2xl font-bold uppercase mb-1 text-center print:text-lg">Experience</h2>

                        {data.workExperience.map(el => (
                            <div key={el.id} className="flex flex-col gap-1 p-4 md:p-5 print:py-3 print:px-4">
                    ...
                    {data.interests.length > 0 &&
                        <section>
                            <h2 className="text-2xl font-bold text-center uppercase mb-4 print:text-lg">Interests</h2>
                            <ul className="flex gap-3 flex-wrap justify-center print:gap-2">
                                {data.interests.map(el => (
                                    <li key={el.id} className='bg-gray-200 capitalize px-4 py-1 md:px-5 md:py-2 rounded-lg print:px-2 print:py-0.5 print:text-sm'>{el.interest}</li>
                                ))}
                            </ul>
                        </section>
                    }
                ...
                <div className="flex flex-col gap-5">
                    <section className="">
                        <h2 className="text-2xl font-bold text-center uppercase mb-1 print:text-lg">Skills</h2>
                        <ul className="flex flex-row gap-3 mt-3 flex-wrap justify-center print:gap-2">
                            {data.skills.map(el => (
                                <li key={el.id} className="bg-gray-200 capitalize px-4 py-1 md:px-5 md:py-2 rounded-lg print:px-2 print:py-0.5 print:text-sm">{el.skill}</li>
                            ))}
                        </ul>
                    </section>
                    <section className="flex flex-col gap-2 shadow-lg rounded-lg">
                        <h2 className="text-2xl font-bold text-center uppercase mb-1 print:text-lg">Education</h2>
                    ...
                    {data.courses.length > 0 &&
                        <section className="flex flex-col gap-2 shadow-lg rounded-lg">
                            <h2 className="text-2xl font-bold text-center uppercase mb-1 print:text-lg">Courses</h2>
                            {data.courses.map(el => (
                                <div key={el.id} className="flex flex-col gap-1 rounded-lg p-4 md:p-5 print:py-3 print:px-4">
